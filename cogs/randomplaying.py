...
class RandomNowPlaying(commands.Cog):
    def __init__(self, bot):
        ...
        bot.logger.info(f"ready, playing {len(self.config['items'])} games")

    async def _setnowplaying(self):
        await self.bot.wait_until_ready()
        while self.bot.is_ready():
            await self.bot.change_presence(
                activity=discord.Game(name=random.sample(self.config["items"], 1)[0])
            )
            interval = random.randint(
                self.config["intervalmin"], self.config["intervalmax"]
            )
