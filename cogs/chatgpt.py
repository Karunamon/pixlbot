...
class ChatGPT(commands.Cog):
    ...
    def format_conversation(
            self, author_display_name: str, conversation: List[dict]
    ) -> str:
    ...
    async def on_message(self, message: discord.Message):
        ...
        cleaned_message = (
            self.remove_bot_mention(message.content)
        ...
        if user_id not in self.user_conversations:
            self.user_conversations[user_id] = [
                {
                    "role": "system",
                    "content": self.config["system_prompt"] + prompt_suffix,
                }
            ]
        ...
        async with message.channel.typing():
            response = await self.send_request_to_chatgpt(
                self.user_conversations[user_id]
            )
            if response:
                self.user_conversations[user_id].append(
    ...
    @gpt.command(
        name="reset",
        description="Reset your conversation history with ChatGPT",
    ...
    @gpt.command(
        ...
        description="Show your current conversation with ChatGPT",
        guild_ids=util.guilds,
    )
    async def show_conversation(self, ctx):
        ...
        formatted_conversation = self.format_conversation(
            ctx.author.display_name, conversation
        )
        bot_display_name = self.bot.user.display_name
    ...
    @gpt.command(
        ...
        description="Save your current conversation with ChatGPT to a text file",
        guild_ids=util.guilds,
    )
